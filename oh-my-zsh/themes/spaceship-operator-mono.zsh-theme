...
spaceship_host() {
  if [[ -n $SSH_CONNECTION ]]; then
    ...
    [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "${SPACESHIP_PREFIX_DIR}" || echo -n ' '
    # Display machine name
    echo -n "%{$fg_no_bold[green]%}%m%{$reset_color%}"
    # Do not show host prefix if prefixes are disabled
    [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "${SPACESHIP_PREFIX_HOST}" || echo -n ' '
...
spaceship_git_status() {
  ...
  if [[ "$(git rev-parse --is-inside-git-dir 2> /dev/null)" == 'false' ]]; then
    ...
    [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "${SPACESHIP_PREFIX_GIT}" || echo -n ' '
...
spaceship_nvm_status() {
  ...
  [[ ${SPACESHIP_PREFIX_SHOW} == true ]] && echo -n "${SPACESHIP_PREFIX_NVM}" || echo -n ' '
...
spaceship_ruby_version() {
  ...
  [[ $SPACESHIP_PREFIX_SHOW == true ]] && echo -n "${SPACESHIP_PREFIX_RUBY}" || echo -n ' '
